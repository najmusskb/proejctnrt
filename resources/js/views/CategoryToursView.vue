<template>
  <div class="category-tours-view">
    <!-- HEADER / BREADCRUMB -->
    <div class="relative bg-navy pt-[120px] pb-16 px-6 overflow-hidden">
      <div class="absolute inset-0 opacity-25" :style="{ background: 'url(' + getImgUrl(category?.image) + ') center/cover no-repeat' }"></div>
      ...
      <div class="relative max-w-[1280px] mx-auto text-center">
        <nav class="text-[12px] tracking-wide text-gold mb-4 justify-center flex">
          ...
          <router-link to="/tours" class="text-white/60 no-underline hover:text-gold transition-colors">Tours</router-link>
          <span class="mx-2 text-white/40">/</span>
          <span class="text-gold">{{ category?.name }}</span>
        </nav>
        <h1 class="font-playfair text-white font-bold leading-tight text-[clamp(28px,5vw,52px)] mb-3">{{ category?.name }}</h1>
        <p class="text-[15px] text-white/70 max-w-[600px] mx-auto leading-relaxed" v-html="category?.description"></p>
      </div>
    </div>
    ...
    <section class="py-20 px-6 bg-cream min-h-[500px]">
      <div class="max-w-[1280px] mx-auto">
        <div class="mb-10 text-center">
          ...
          <h2 class="font-playfair text-navy font-bold leading-tight text-[clamp(24px,3.5vw,36px)]">Tours in {{ category?.name }}</h2>
        ...
        <div v-else class="text-center py-10 bg-white rounded-2xl border border-cream-dark shadow-sm">
          ...
          <p class="text-[#6b7280] text-[14px]">We currently do not have any tours available in this category.</p>
...
@media (max-width: 480px) {
  ...
  .category-tours-view section { padding-top: 48px; padding-bottom: 48px; }
}
</style>
