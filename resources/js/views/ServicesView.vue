      ...
      <div v-else class="Srvc-grid">
        ...
        >
          <div class="Srvc-card-img">
            <img :src="getImgUrl(srv.image)" :alt="srv.name || srv.title">
            <div class="Srvc-icon" v-html="srv.icon || '&#10024;'"></div>
          </div>
          <div class="Srvc-card-body">
            <h3 class="Srvc-card-title">{{ srv.name || srv.title }}</h3>
            <p class="Srvc-card-desc">{{ srv.short_description || srv.description }}</p>
...
.Srvc-hero-content { position: relative; z-index: 2; max-width: 620px; }
...
@media (max-width: 768px) { .Srvc-grid { grid-template-columns: 1fr; } .Srvc-title { font-size: 40px; } .Srvc-hero { height: 320px; background-attachment: scroll; } .Srvc-wrap { padding: 40px 16px 50px; } }
@media (max-width: 480px) { .Srvc-title { font-size: 30px; } .Srvc-subtitle { font-size: 15px; } .Srvc-hero { height: 280px; padding: 0 4%; } .Srvc-wrap { padding: 30px 12px 40px; } .Srvc-card-img { height: 130px; } }
.Srvc-card {
